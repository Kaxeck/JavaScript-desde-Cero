...
function guardarEnLocalStorage() {
    // Guardamos el HTML actual del contenedor de comentarios
    let contenido = document.querySelector('.comentarios').innerHTML;
    localStorage.setItem('comentariosGuardados', contenido);
}

// Cargar los comentarios cuando la página se carga
window.addEventListener('DOMContentLoaded', function () {
    let contenidoGuardado = localStorage.getItem('comentariosGuardados');
    if (contenidoGuardado) {
        document.querySelector('.comentarios').innerHTML = contenidoGuardado;
    }
});